...
import { Monitor, Server, Shield, Database, Globe, User, Target, Wifi, HardDrive, Lock, Trash2, HelpCircle, GripVertical, Clock } from 'lucide-react';
import TimelineEvent from './TimelineEvent';
import QuestionMarkNode from './QuestionMarkNode';

// Timeline View Component
const TimelineView = ({ events, timelineSortOrder, selectedNode, onNodeSelect, onNodeUpdate }) => {
  const [timelineEvents, setTimelineEvents] = useState([]);
  const [draggedIndex, setDraggedIndex] = useState(null);
  const containerRef = useRef(null);
  const listRef = useRef(null);
  const itemRefs = useRef([]);
  const lastSwapIndexRef = useRef(null);
  ...
  }, [events, timelineSortOrder, draggedIndex]);
  ...
  // Reset drag state when selected node changes
  useEffect(() => {
    if (selectedNode && draggedIndex !== null) {
      // If user selects a different node while dragging, cancel the drag
      const selectedIndex = timelineEvents.findIndex(e => e.id === selectedNode.id);
  ...
  }, [selectedNode, draggedIndex, timelineEvents]);
  ...
  const handleDragEnd = React.useCallback(() => {
    ...
    if (onNodeUpdate && timelineEvents.length > 0) {
      setTimeout(() => {
        onNodeUpdate({ events: timelineEvents });
      }, 50);
    }
  }, [onNodeUpdate, timelineEvents]);

  const handleDragMove = React.useCallback((dragInfo) => {
    if (draggedIndex === null || !listRef.current) return;
    if (isSwappingRef.current) return; // Prevent swaps during animation
    
    const items = Array.from(listRef.current.querySelectorAll('[data-timeline-event]')) as HTMLElement[];
    if (items.length === 0 || !items[draggedIndex]) return;

    // Get the dragged item's current position
    const draggedRect = items[draggedIndex].getBoundingClientRect();
    const draggedCenterY = draggedRect.top + draggedRect.height / 2;

    let swapWithIndex = null;
    let closestDistance = Infinity;

    // Find the closest item we should swap with
    for (let i = 0; i < items.length; i++) {
      ...
      // Moving up - swap when dragged center is significantly above target center
      if (i < draggedIndex && draggedCenterY < itemCenterY - 30) {
        if (distance < closestDistance) {
          closestDistance = distance;
          swapWithIndex = i;
        }
      }
      // Moving down - swap when dragged center is significantly below target center  
      else if (i > draggedIndex && draggedCenterY > itemCenterY + 30) {
        if (distance < closestDistance) {
          closestDistance = distance;
          swapWithIndex = i;
        }
    ...
    // Only swap if we found a target and it's different from last swap
    if (swapWithIndex !== null && swapWithIndex !== draggedIndex && swapWithIndex !== lastSwapIndexRef.current) {
      isSwappingRef.current = true;
      lastSwapIndexRef.current = swapWithIndex;

      setTimelineEvents(prevEvents => {
        const newEvents = [...prevEvents];
        const [removed] = newEvents.splice(draggedIndex, 1);
        newEvents.splice(swapWithIndex, 0, removed);
        return newEvents;
      });
      setDraggedIndex(swapWithIndex);
      
      // Allow next swap after animation completes
      setTimeout(() => {
        isSwappingRef.current = false;
        lastSwapIndexRef.current = null;
      }, 200);
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-8">
        <div className="max-w-md text-center">
          ...
          <h2 className="text-2xl font-semibold text-white mb-3">No Timeline Events</h2>
          <p className="text-slate-400 text-sm leading-relaxed">
            Timeline events will appear here once the incident analysis is complete. 
            These events represent the chronological sequence of the attack.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-950 overflow-y-auto">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-12 text-center">
          <h2 className="text-3xl font-bold text-white mb-3">Attack Timeline</h2>
          <p className="text-slate-400 text-sm max-w-md mx-auto">
            Chronological sequence of attack events. Drag events to reorder, click to select, or edit timestamps.
          </p>
          <div className="mt-4 flex flex-col items-center gap-3">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-cyan-400 rounded-full"></div>
              <span className="text-xs text-slate-500">{timelineEvents.length} {timelineEvents.length === 1 ? 'event' : 'events'}</span>
            </div>
        ...
        {/* Timeline Container with constraints */}
        <div className="relative min-h-screen" ref={containerRef}>
          {/* Main timeline line */}
          <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-cyan-500/50 via-slate-600 to-slate-800"></div>
          ...
          {draggedIndex !== null && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="fixed top-20 left-1/2 transform -translate-x-1/2 z-50 bg-cyan-500/20 backdrop-blur-md border border-cyan-500/30 rounded-xl px-4 py-2 shadow-lg"
            >
          ...
          <div ref={listRef} className="relative py-8 flex flex-col gap-8">
          ...
          <div className="mt-16 pb-8 flex justify-center">
            <div className="inline-flex items-center gap-3 px-6 py-3 bg-slate-800/50 rounded-2xl border border-slate-700/50 backdrop-blur-sm hover:bg-slate-800/70 transition-all">
              ...
              <div className="flex flex-col">
                <span className="text-sm text-slate-300 font-medium">Timeline Analysis Complete</span>
                <span className="text-xs text-slate-500">{timelineEvents.length} events processed</span>
              </div>
            </div>
          </div>
        </div>
...
export default function DiagramCanvas({ 
    ...
    if (diagramData && flowType === 'network_map' && diagramData.nodes) {
      setPositions(prev => {
        ...
        if (nodesWithoutPos.length === nodes.length) {
          ...
          let maxLevel = Object.values(level).length ? Math.max(...Object.values(level) as number[]) : 0;
          nodes.forEach(n => {
            if (!(n.id in level)) {
              maxLevel += 1;
              level[n.id] = maxLevel;
          ...
          Object.entries(level).forEach(([nodeId, lv]) => {
            const levelNum = lv as number;
            groups[levelNum] = groups[levelNum] || [];
            groups[levelNum].push(nodeId);
          });
          ...
          const existingPos = Object.values(newPositions)[0] as { x: number; y: number } | undefined;
          if (existingPos) {
  ...
  const handleMouseUp = React.useCallback((e) => {
    ...
    if (dragEdgeStart) {
      ...
      if (svgRef.current && positions) {
        ...
        for (const [nodeId, pos] of Object.entries(positions) as [string, { x: number; y: number }][]) {
      ...
      return (
        <div className="flex items-center justify-center h-full text-slate-400">
          <div className="text-center">
            <div className="text-lg mb-2">No network map data available</div>
            <div className="text-sm text-slate-500">Expected network nodes but found none</div>
          </div>
        </div>
      );
    }
  ...
  if (flowType === 'timeline') {
    if (!diagramData || !diagramData.events) {
      return (
        <div className="flex items-center justify-center h-full text-slate-400">
          <div className="text-center">
            <div className="text-lg mb-2">No timeline data available</div>
            <div className="text-sm text-slate-500">Expected timeline events but found none</div>
          </div>
        </div>
      );
    }
  ...
  if (flowType === 'mitre_attack') {
    if (!diagramData || !diagramData.tactics) {
      return (
        <div className="flex items-center justify-center h-full text-slate-400">
          <div className="text-center">
            <div className="text-lg mb-2">No MITRE ATT&CK data available</div>
            <div className="text-sm text-slate-500">Expected tactics and techniques but found none</div>
          </div>
        </div>
      );
    }
