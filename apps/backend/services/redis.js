...
class RedisService {
  ...
  async ensureConnected(maxRetries = 5) {
    ...
    while (!this.client.isOpen && attempt < maxRetries) {
      try {
        ...
        console.error(`Redis connect attempt ${attempt}/${maxRetries} failed:`, err);
  ...
  async setSession(sessionId, username, ttl = 3600, apiToken = null, confluenceUrl = null) {
    await this.ensureConnected();
  ...
  async getSessionUsername(sessionId) {
    await this.ensureConnected();
  ...
  async saveUserFlow(username, flowData) {
    const flowsKey = `user:flows:${username}`;
  ...
  async deleteUserFlow(username, flowId) {
    const flowsKey = `user:flows:${username}`;
    const flows = await this.getUserFlows(username);
